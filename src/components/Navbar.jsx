...
function Navbar() {
  ...
  return (
    <div className={`fixed z-[999] w-full  px-10 md:px-20 py-8 font-['Neue Montreal] flex flex-row items-center justify-between ${open ? 'bg-zinc-900': null}`}>
      ...
      <div className={`ham md:hidden ${open}`} onClick={showNavBar}>
        <svg
          className={`nav-toggle ${open ? "cross" : ""}`}
          width="24"
          height="24"
          viewBox="0 0 24 24"
          fill="none"
        ...
        >
          {open ? (
            <>
              ...
              <line x1="0" y1="24" x2="24" y2="0" stroke="currentColor"></line>
            </>
          ) : (
            <>
              <line x1="0" y1="6" x2="24" y2="6" stroke="currentColor"></line>
              <line x1="0" y1="12" x2="24" y2="12" stroke="currentColor"></line>
              <line x1="0" y1="18" x2="24" y2="18" stroke="currentColor"></line>
            </>
          )}
        </svg>
      </div>

      <div className={`md:flex md:items-center md:pb-0 pb-12 absolute md:static bg-zinc-900 md:bg-transparent md:z-auto z-[-1] left-0 w-full md:w-auto md:pl-0 pl-9 transition-all duration-500 ease-in ${open ? 'top-20 ':'top-[-490px]'}`}>
        {Links.map(
          (link, index) => (
